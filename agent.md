...
4. `MINIO_API_PORT=9100 GENOME_HUB_PORT=3106 HAPPY_SERVER_PORT=3105 KANBAN_PORT=8083 ../scripts/local-smoke.sh`
5. `docker compose -f ../docker-compose.yml -f ../docker-compose.better0330.yml --env-file ../.env.better0330 run --rm --no-deps --entrypoint /bin/sh aha-cli -lc 'printf "AHA_HOME_DIR=%s\n" "$AHA_HOME_DIR" && node /app/bin/aha-v9.mjs --version'`
6. 浏览器验证 `http://localhost:8083`
